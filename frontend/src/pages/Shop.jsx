...
const Shop = ({ onAddToCart }) => {
  ...
  const colorOptions = useMemo(() => {
    const colors = catalog60.map((product) => product.color).filter(Boolean);
    return ["all", ...new Set(colors)];
  }, []);

  const badgeOptions = useMemo(() => {
    const badges = catalog60.map((product) => product.badge).filter(Boolean);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.badge !== "all") {
      products = products.filter(
        (product) =>
          product.badge?.toLowerCase() === filters.badge.toLowerCase()
      );
    }
    ...
    if (filters.price !== "all") {
      products = products.filter((product) => {
        ...
        if (filters.price === "under-25000") return price < 25000;
        if (filters.price === "25000-50000") return price >= 25000 && price <= 50000;
        if (filters.price === "50000-80000") return price > 50000 && price <= 80000;
        if (filters.price === "over-80000") return price > 80000;
        return true;
      });
    }
    ...
    return products;
  }, [currentCategory, searchQuery, filters]);
  ...
  const clearFilters = () => {
    setFilters({ color: "all", badge: "all", price: "all", sort: "featured" });
    setSearchQuery("");
  };
  ...
  const handleCompleteLook = (productOrId) => {
    ...
    const foundProduct = catalog60.find((product) => String(product.id) === String(productOrId));
    if (foundProduct) setCompleteLookProduct(foundProduct);
  };
    ...
    <main className="min-h-screen bg-[#080808] text-white">
        ...
        <div className="relative mx-auto max-w-7xl px-5 py-16 sm:px-8 sm:py-20 lg:px-10">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 max-w-2xl text-sm leading-7 text-white/55 sm:text-base">
              Discover curated fashion pieces selected through AI-powered style intelligence, premium design, and modern haute couture aesthetics.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4 sm:p-5">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <FilterSelect
                  label="Color"
                  value={filters.color}
                  onChange={(value) => setFilters((p) => ({ ...p, color: value }))}
                  options={colorOptions}
                />
                <FilterSelect
                  label="Collection"
                  value={filters.badge}
                  onChange={(value) => setFilters((p) => ({ ...p, badge: value }))}
                ...
                <FilterSelect
                  ...
                  options={["all", "under-25000", "25000-50000", "50000-80000", "over-80000"]}
                  labels={{
                    all: "All Prices",
                    "under-25000": "Under LKR 25,000",
                    "25000-50000": "LKR 25,000 – 50,000",
                    "50000-80000": "LKR 50,000 – 80,000",
                    "over-80000": "Over LKR 80,000",
                  }}
                />
                <FilterSelect
                  label="Sort By"
                  ...
                  options={["featured", "price-low", "price-high", "name"]}
                  labels={{
                    featured: "Featured",
              ...
              {activeFilterCount > 0 && (
                <div className="mt-4 flex justify-end">
                  <button type="button" onClick={clearFilters} className="text-xs text-white/45 underline underline-offset-4 transition hover:text-white">
                    Clear all filters
                  </button>
                </div>
              )}
      ...
      <section className="mx-auto max-w-7xl px-5 pt-10 sm:px-8 lg:px-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-white/35">Curated Selection</p>
        ...
        {filteredProducts.length > 0 ? (
          ...
          <div className="flex min-h-[420px] items-center justify-center rounded-3xl border border-white/10 bg-white/[0.02] px-6 text-center">
            <div className="max-w-md">
              <h3 className="font-serif text-2xl text-white">No pieces found</h3>
              <p className="mt-3 text-sm leading-6 text-white/45">We couldn't find anything matching your current search and filters.</p>
              <button type="button" onClick={clearFilters} className="mt-6 rounded-full bg-white px-6 py-3 text-sm font-medium text-black transition hover:bg-white/90">
                Reset Collection
              </button>
            </div>
          </div>
...
const FilterSelect = ({ label, value, onChange, options, labels = {} }) => {
  return (
    <div>
      <label className="mb-2 block text-[10px] uppercase tracking-[0.18em] text-white/35">{label}</label>
      <div className="relative">
        ...
        <ChevronDown size={16} className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-white/35" />
      </div>
    </div>
  );
};
